  ...
  public = list(
    ...
    fromJSON = function(AccountDirectoriesJson) {
      ...
      if (!is.null(AccountDirectoriesObject$`accounts`)) {
        self$`accounts` <- ApiClient$new()$deserializeObj(AccountDirectoriesObject$`accounts`, "array[character]", "package:factset.analyticsapi.engines.v2")
      }
      if (!is.null(AccountDirectoriesObject$`directories`)) {
        self$`directories` <- ApiClient$new()$deserializeObj(AccountDirectoriesObject$`directories`, "array[character]", "package:factset.analyticsapi.engines.v2")
      }
    },
