  ...
  public = list(
    ...
    fromJSON = function(PAComponentJson) {
      ...
      if (!is.null(PAComponentObject$`accounts`)) {
        self$`accounts` <- ApiClient$new()$deserializeObj(PAComponentObject$`accounts`, "array[PAIdentifier]", "package:factset.analyticsapi.engines.v2")
      }
      if (!is.null(PAComponentObject$`benchmarks`)) {
        self$`benchmarks` <- ApiClient$new()$deserializeObj(PAComponentObject$`benchmarks`, "array[PAIdentifier]", "package:factset.analyticsapi.engines.v2")
      }
