...
main <- function() {
  ...
  apiResponse <-
    GetApiResponse("POST",
                   ...
                   calcRequestParameters,
                   customHeaders)
  # If the status code returns as "201" return the result
  if (status_code(apiResponse) == "201")
  {
    # Convert stach data into data frame and print the result
    stachdata <- ProcessCalcUnitApiResponse(apiResponse)
    dataFrameResult <- ConvertStachToDataframe(stachdata)
    PrintDataFrameResult(dataFrameResult)
  } else if (status_code(apiResponse) == "200")
  {
    # Calculation Unit Id Failed
    responseObj <-
      jsonlite::fromJSON(httr::content(apiResponse, "text", encoding = "UTF-8"))
  ...
  } else if (status_code(apiResponse) == "202")
  {
    ...
    # Polling till the calculation is completed
    responseObj <-
      jsonlite::fromJSON(httr::content(apiResponse, "text", encoding = "UTF-8"))
